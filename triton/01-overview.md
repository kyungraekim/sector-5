...
### Python JIT Decorator (`@triton.jit`)
...
Triton IR (produced from Python AST via MLIR)
{% endstep %}

{% step %}
### TTGIR

TritonGPU IR — GPU-specific layouts and optimizations
{% endstep %}
...
Binary produced via `ptxas` assembler
{% endstep %}
{% endstepper %}
...
* Stores metadata (num\_warps, shared\_memory, registers)
...
{% step %}
### JITFunction.run() (jit.py:708-763)
...
{% step %}
### Compilation (compiler.py:226-363)
...
{% step %}
### CompiledKernel initialization (compiler.py:439-474)
...
{% step %}
### Kernel execution (driver.c:570-636)

* Arguments extracted from Python objects (tensor.data\_ptr())
...
* Transform Python code into GPU binary
* Cache compiled kernels for reuse
* Marshal Python arguments to C-compatible format
* Call these CUDA APIs with the right parameters
